...
int Resampler::initObjects() {
    ...
    resampleCtx = swr_alloc_set_opts(resampleCtx,
                               ad->pOutCodecContext->channel_layout,
                               ad->pOutCodecContext->sample_fmt,
                               ad->pOutCodecContext->sample_rate,
                               ad->pInCodecContext->channel_layout,
...
//    resp = av_samples_alloc_array_and_samples(&srcData, &srcLineSize, srcNumChannels, srcNumSamples,
//                                              ad->pInCodecContext->sample_fmt, 0);
//    if (resp < 0) {
//        cout << "error: could not allocate source samples:" << endl;
//        exit(1);
//    }
//    maxDstNumSamples = dstNumSamples = av_rescale_rnd(srcNumSamples, ad->pInCodecContext->sample_rate,
//                                                      ad->pOutCodecContext->sample_rate, AV_ROUND_UP);
//
//    dstNumChannels = ad->pOutCodecContext->channels;
//    resp = av_samples_alloc_array_and_samples(&dstData, &dstLineSize, dstNumChannels, dstNumSamples,
//                                              ad->pInCodecContext->sample_fmt, 0);
//    if (resp < 0) {
